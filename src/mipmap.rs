...
pub fn max_mipmaps(texture_size: Extent3d) -> u32 {
    (texture_size.width as f32).log2().max((texture_size.width as f32).log2()).floor() as u32 + 1
}
    ...
    pub fn generate_mipmaps(&self, device: &Device, encoder: &mut CommandEncoder, texture: &Texture) {
        ...
        for bind_group in &bind_groups {
            ...
            let local_size: u32 = 8;
            {
                ...
                cpass.dispatch((width+local_size-1)/local_size, (height+local_size-1)/local_size, 1);
            }
        }
    }
}