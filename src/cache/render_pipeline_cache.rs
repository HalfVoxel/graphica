...
use wgpu::{BlendState, ColorWrite, DepthStencilState, Device, MultisampleState, PipelineLayout, PrimitiveState, RenderPipeline, RenderPipelineDescriptor, ShaderModule, TextureFormat};
...
impl RenderPipelineCache {
    pub fn get<'a>(&'a mut self, device: &Device, key: RenderPipelineKey) -> &'a CachedRenderPipeline {
        ...
        self.cache.entry(key).or_insert_with_key(|key| {
            let pipeline = key.base.to_wgpu_pipeline(device, &[wgpu::ColorTargetState {
            ...
            Arc::new(CachedRenderPipelineImpl {
                pipeline
            })
        })
    }
...
impl RenderPipelineBase {
    fn to_wgpu_pipeline(&self, device: &Device, targets: &[wgpu::ColorTargetState], depth_stencil: Option<DepthStencilState>, multisample: MultisampleState) -> RenderPipeline {
