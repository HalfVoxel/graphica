...
impl Material {
    ...
    pub fn new(device: &Device, label: String, bind_group_layout: Arc<BindGroupLayout>, bindings: Vec<BindGroupEntryArc>) -> Material {
        let bind_group = None; //Self::bind_group_from_entries(device, &label, &bind_group_layout, &bindings).map(Rc::new);
        Material {
    ...
    pub fn modified(&self, device: &Device, overrides: &[BindGroupEntryArc]) -> Material {
        ...
        let bind_group = None; //Self::bind_group_from_entries(device, &self.label, &self.bind_group_layout, &new_bindings).map(Rc::new);
...
pub enum BindingResourceArc {
    ...
    // Sampler(Option<ByAddress<Arc<u64>>>),
...
impl BindingResourceArc {
    ...
    pub fn sampler(sampler: Option<Arc<Sampler>>) -> Self {
        // Self::Sampler(sampler.map(Box::new).map(ByAddress::from))
        Self::texture(None)
    }
...
impl MaterialCache {
    fn hash_material(material: &Material, changes: &[BindGroupEntryArc]) -> u64 {
        ...
        // material.bind_group_layout.hash(&mut hasher);
        // let mut j = 0;
